<div align="center">
  <img width="1000" alt="banner principal" src="pablo(assets)/Samuraicyber.png" />
</div>

---

<div align="center">
  <em>Serenidade para pensar. Intensidade para agir.<br>
  Disciplina para evoluir. Consistência para vencer.</em>
</div>

---

## 👨‍💻 Sobre mim

<img align="right" src="pablo(assets)/banner3.jpg" width="300" alt="Banner perfil">

**Pablo Martins**  

Desenvolvedor Full Stack Júnior focado em construção de APIs, automação e sistemas escaláveis.

Tenho como base a disciplina do **Kaizen (melhoria contínua)** e o conceito **Shu-Ha-Ri (aprendizado progressivo)** — aplicando consistência e evolução constante na minha jornada como desenvolvedor.

💡 Transformo problemas em soluções práticas  
🚀 Busco evolução técnica todos os dias  
🌧️ Código limpo, mente focada, execução consistente  

---

## 🔭 O que você vai encontrar aqui

- 🚀 Projetos Full Stack (Node.js, Python, APIs REST)
- ⚙️ Ferramentas de automação
- 📚 Estudos práticos de programação
- 💡 Conteúdo voltado para evolução como dev

---

## 🚀 Skills

### 💻 Linguagens
<div>
  <img src="https://cdn.simpleicons.org/javascript/F7DF1E" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="40"/>
</div>

### 🌐 Front-end
<div>
  <img src="https://cdn.simpleicons.org/html5/E34F26" height="40"/>
  <img src="https://github.com/tandpfun/skill-icons/blob/main/icons/CSS.svg" height="40"/>
</div>

### ⚙️ Back-end
<div>
  <img src="https://cdn.simpleicons.org/nodedotjs/339933" height="40"/>
  <img src="https://cdn.simpleicons.org/fastapi/339933%22" height="40"/>
  <img src="https://cdn.simpleicons.org/flask/F7DF1E" width="40">
</div>

### 🗄️ Banco de Dados
<div>
  <img src="https://github.com/tandpfun/skill-icons/blob/main/icons/MySQL-Light.svg" height="40"/>
</div>

### 🛠️ Ferramentas
<div>
  <img src="https://cdn.simpleicons.org/git/F05032" height="40"/>
  <img src="https://github.com/tandpfun/skill-icons/blob/main/icons/Linux-Light.svg" height="40"/>
  <img src="https://github.com/tandpfun/skill-icons/blob/main/icons/Github-Light.svg" height="40"/>
</div>

---

## 🌐 Conecte-se comigo

<div align="center">
  <a href="https://www.linkedin.com/in/pablo-martins-02bb81326/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://www.instagram.com/pablo.martin031" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-000000?style=for-the-badge&logo=instagram&logoColor=white"/>
  </a>
  <a href="https://github.com/PabloMartins031" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-24292E?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</div>

---

<div align="center">
  <strong>💻 Aqui o código não é só escrito — ele evolui.</strong><br>
  <em>Explore, aprenda e construa junto.</em>
</div>
